...
FASHION_MNIST::FASHION_MNIST(const std::string& root, Mode mode)
    : images_(read_images(root, mode == Mode::kTrain)),
      targets_(read_targets(root, mode == Mode::kTrain)) {}

Example<> FASHION_MNIST::get(size_t index) {
  return {images_[index], targets_[index]};
}

optional<size_t> FASHION_MNIST::size() const {
  return images_.size(0);
}

bool FASHION_MNIST::is_train() const noexcept {
  return images_.size(0) == kTrainSize;
}

const Tensor& FASHION_MNIST::images() const {
  return images_;
}

const Tensor& FASHION_MNIST::targets() const {
  return targets_;
}
